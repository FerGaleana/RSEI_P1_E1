...
static uint16_t g_short_address = 0;
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn, uint8_t appInstance)
{
	mlmeMessage_t *pMsg;
//  mlmeAssociateRes_t *g_pAssocRes;
	uint8_t counter_from_past_nodes = 0;
	static boolean_t flag_node_da_saved = FALSE;
...
//    	g_pAssocRes->assocShortAddress = 0x0001;
			...
			/* Assign a unique short address less than 0xfffe if the device requests so. */
			if(PRIMERNODO == g_node)
			{
				g_pAssocRes->assocShortAddress = 0x0003;
				g_short_address = g_pAssocRes->assocShortAddress;
			}
			else
			{
				g_short_address += 0x0010;
				g_pAssocRes->assocShortAddress = g_short_address;
			}
		}
		else
		{
	...
	//node_info[g_node].shortAddress = g_pAssocRes->assocShortAddress;
	node_info[g_node].RxOnWhenIdle = pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoRxWhenIdle_c;

	if(pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoDeviceFfd_c)
	{
		node_info[g_node].deviceType = FFD;
	}
	else
	{
		node_info[g_node].deviceType = RFD;
	}

	// Esta se puso aqui para que el Extended Address se pudiera guardar
	// de una sin tener que esperar al siguiente nodo a que se conecte
	node_info[g_node].extendedAddress = g_pAssocRes->deviceAddress;

    for(counter_from_past_nodes = FALSE; counter_from_past_nodes < g_node; counter_from_past_nodes++)
    {
    	if(node_info[counter_from_past_nodes].extendedAddress == g_pAssocRes->deviceAddress)
    	{
    		g_pAssocRes->assocShortAddress = node_info[counter_from_past_nodes].shortAddress;
    		flag_node_da_saved = TRUE;
    	}
    } /* end of for */

	if(!flag_node_da_saved)
	{
		node_info[g_node].shortAddress = g_pAssocRes->assocShortAddress;
		g_node++;
	}
